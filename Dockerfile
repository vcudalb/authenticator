FROM mcr.microsoft.com/dotnet/runtime:7.0 AS base
WORKDIR /app

FROM mcr.microsoft.com/dotnet/sdk:7.0 AS build
WORKDIR /src
COPY ["src/Authenticator.Api/Authenticator.Api.csproj", "src/Authenticator.Api/"]
RUN dotnet restore "src/Authenticator.Api/Authenticator.Api.csproj"
COPY . .
WORKDIR "/src/src/Authenticator.Api"
RUN dotnet build "Authenticator.Api.csproj" -c Release -o /app/build

FROM build AS publish
RUN dotnet publish "Authenticator.Api.csproj" -c Release -o /app/publish /p:UseAppHost=false

FROM base AS final
WORKDIR /app
COPY --from=publish /app/publish .
ENTRYPOINT ["dotnet", "Authenticator.Api.dll"]
